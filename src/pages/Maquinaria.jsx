...
export default function Maquinaria({ inventario, curUBO, raw }) {
  ...
  const vp     = invFiltrado.filter(e => e.clasificacion === 'VP')
  ...
  return (
    <div className="p-3 sm:p-4 space-y-4 bg-slate-50 min-h-screen">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          {l:'Total inventario', v:total, s:'equipos', c:'border-t-blue-600', vc:'text-blue-700'},
          {l:'Disponibles',      v:disp,  s:`${total?(disp/total*100).toFixed(0):'0'}% libre`, c:'border-t-green-600', vc:'text-green-700'},
          {l:'En uso',           v:enUso, s:`${total?(enUso/total*100).toFixed(0):'0'}% asignado`, c:'border-t-red-500', vc:'text-red-700'},
          {l:'MP — Maq. Pesada', v:mp.length, s:`${mpDisp} disponibles`, c:'border-t-amber-600', vc:'text-amber-700'},
          {l:'VP — Veh. Pesado', v:vp.length, s:`${vpDisp} disponibles`, c:'border-t-blue-500', vc:'text-blue-700'},
        ].map(({l,v,s,c,vc}) => (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-[#1F3864]">
                {['UBO','MP Total','MP Uso','MP Disp.','VP Total','VP Uso','VP Disp.','Total','% Uso'].map(h=>(

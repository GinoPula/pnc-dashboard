export function FilterBar({ ubos, deps, anios, curAnio, curMes, curUBO, curDep, curTipo, setCurAnio, setCurMes, setCurUBO, setCurDep, setCurTipo, count, total }) {
  const sel = 'text-xs border border-slate-300 border-slate-300 rounded-md px-2 py-1 bg-white bg-white text-slate-700 cursor-pointer max-w-[120px] sm:max-w-[150px]'
      ...
      <div className="flex gap-1">
        {TIPOS.map(t=>(
          <button key={t} onClick={()=>setCurTipo(t)}
            className={`px-1.5 sm:px-2 py-0.5 rounded-full text-xs border transition-all whitespace-nowrap ${curTipo===t?'bg-[#CC1C2C] text-white border-[#CC1C2C] font-bold':'bg-transparent text-slate-500 border-slate-300 hover:border-blue-400'}`}>
            {TIPO_LABELS[t]}
          </button>
        ))}
      </div>
